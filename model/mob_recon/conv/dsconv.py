...
class DSConv(nn.Module):
    ...
    def forward(self, x):
        # print("input:", x.shape)  # torch.Size([8, 778, 32])
        n_nodes, _ = self.indices.size()
        bs = x.size(0)
        x = torch.index_select(x, self.dim, self.indices.to(x.device).view(-1)) # torch.Size([32, 7002, 32])
        x = x.view(bs * n_nodes, self.seq_length, -1).transpose(1, 2) # torch.Size([24896, 32, 9])
        x = x.view(x.size(0), x.size(1), int(np.sqrt(self.seq_length)), int(np.sqrt(self.seq_length))) # torch.Size([24896, 32, 3, 3])
        x = self.spatial_layer(x).view(bs, n_nodes, -1) # torch.Size([32, 778, 32])
        x = self.channel_layer(x) # torch.Size([32, 778, 3])
        # print("output:", x.shape)  # torch.Size([8, 778, 3])
