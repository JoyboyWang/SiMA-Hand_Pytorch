...
def qslim_decimator_transformer(mesh, factor=None, n_verts_desired=None):
    ...
    def collapse_cost(Qv, r, c, v):
        ...
        result = {'destroy_c_cost': destroy_c_cost, 'destroy_r_cost': destroy_r_cost, 'collapse_cost': min([destroy_c_cost, destroy_r_cost]), 'Qsum': Qsum}
        return result
...
def downsample_vertices(vertices, faces, factors):
    """Generates len(factors) meshes, each of them is scaled by factors[i] and
       computes the transformations between them.
    ...
    #     mesh.export("./debug_downsample/hand_{}.obj".format(m.v.shape[0]))
